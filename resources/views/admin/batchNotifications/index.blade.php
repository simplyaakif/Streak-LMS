...
@can('batch_notification_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.batch-notifications.create') }}">
                        ...
                        <tr data-entry-id="{{ $batchNotification->id }}">
                            ...
                            <td>
                                @can('batch_notification_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.batch-notifications.show', $batchNotification->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('batch_notification_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.batch-notifications.edit', $batchNotification->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('batch_notification_delete')
                                    <form action="{{ route('admin.batch-notifications.destroy', $batchNotification->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('batch_notification_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.batch-notifications.massDestroy') }}",
