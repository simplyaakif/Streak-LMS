        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.queries.store") }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('courses') ? 'is-invalid' : '' }}"
                            name="courses[]" id="courses" multiple>
                        @foreach($courses as $id => $courses)
                            <option
                                value="{{ $id }}" {{ in_array($id, old('courses', [])) ? 'selected' : '' }}>{{ $courses }}</option>
                        @endforeach
                    </select>
                ...
                @if(Auth::user()->isAdminOrDirector())
                    <div class="form-group">
                        ...
                        <select class="form-control select2 {{ $errors->has('dealt_by') ? 'is-invalid' : '' }}"
                                name="dealt_by_id" id="dealt_by_id" required>
                            @foreach($dealt_bies as $id => $dealt_by)
                                <option
                                    value="{{ $id }}" {{ old('dealt_by_id') == $id ? 'selected' : '' }}>{{ $dealt_by }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('comments_remarks') ? 'is-invalid' : '' }}"
                              name="comments_remarks" id="comments_remarks">{{ old('comments_remarks') }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('interaction_type') ? 'is-invalid' : '' }}"
                            name="interaction_type_id" id="interaction_type_id">
                        @foreach($interaction_types as $id => $interaction_type)
                            <option
                                value="{{ $id }}" {{ old('interaction_type_id') == $id ? 'selected' : '' }}>{{ $interaction_type }}</option>
                        @endforeach
                    </select>
