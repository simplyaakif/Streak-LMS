                ...
                <tbody>
                    @foreach($smsTemplates as $key => $smsTemplate)
                        <tr data-entry-id="{{ $smsTemplate->id }}">
                            ...
                            <td>
                                <span style="display:none">{{ $smsTemplate->active ?? '' }}</span>
                                <input type="checkbox" disabled="disabled" {{ $smsTemplate->active ? 'checked' : '' }}>
                            </td>
                            <td>
                                @can('sms_template_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.sms-templates.show', $smsTemplate->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan
                                ...
                                @can('sms_template_delete')
                                    <form action="{{ route('admin.sms-templates.destroy', $smsTemplate->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
