    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.email-templates.update", [$emailTemplate->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $emailTemplate->title) }}">
                @if($errors->has('title'))
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('email_message') ? 'is-invalid' : '' }}" name="email_message" id="email_message">{!! old('email_message', $emailTemplate->email_message) !!}</textarea>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('active') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="active" id="active" value="1" {{ $emailTemplate->active || old('active', 0) === 1 ? 'checked' : '' }}>
                    <label class="form-check-label" for="active">{{ trans('cruds.emailTemplate.fields.active') }}</label>
                </div>
