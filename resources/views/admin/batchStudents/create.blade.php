    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.batch-students.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('sessions_start_date') ? 'is-invalid' : '' }}" type="text" name="sessions_start_date" id="sessions_start_date" value="{{ old('sessions_start_date') }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('session_end_date') ? 'is-invalid' : '' }}" type="text" name="session_end_date" id="session_end_date" value="{{ old('session_end_date') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('student_status') ? 'is-invalid' : '' }}" name="student_status_id" id="student_status_id" required>
                    @foreach($student_statuses as $id => $student_status)
                        <option value="{{ $id }}" {{ old('student_status_id') == $id ? 'selected' : '' }}>{{ $student_status }}</option>
                    @endforeach
                </select>
