...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.student-statuses.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="status_title">{{ trans('cruds.studentStatus.fields.status_title') }}</label>
                <input class="form-control {{ $errors->has('status_title') ? 'is-invalid' : '' }}" type="text" name="status_title" id="status_title" value="{{ old('status_title', '') }}" required>
