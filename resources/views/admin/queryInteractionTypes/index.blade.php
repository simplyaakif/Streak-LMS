...
@can('query_interaction_type_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.query-interaction-types.create') }}">
                        ...
                        <tr data-entry-id="{{ $queryInteractionType->id }}">
                            ...
                            <td>
                                @can('query_interaction_type_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.query-interaction-types.show', $queryInteractionType->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('query_interaction_type_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.query-interaction-types.edit', $queryInteractionType->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('query_interaction_type_delete')
                                    <form action="{{ route('admin.query-interaction-types.destroy', $queryInteractionType->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('query_interaction_type_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.query-interaction-types.massDestroy') }}",
