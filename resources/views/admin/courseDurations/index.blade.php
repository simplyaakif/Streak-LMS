...
@can('course_duration_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.course-durations.create') }}">
                        ...
                        <tr data-entry-id="{{ $courseDuration->id }}">
                            ...
                            <td>
                                @can('course_duration_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.course-durations.show', $courseDuration->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('course_duration_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.course-durations.edit', $courseDuration->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('course_duration_delete')
                                    <form action="{{ route('admin.course-durations.destroy', $courseDuration->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('course_duration_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.course-durations.massDestroy') }}",
