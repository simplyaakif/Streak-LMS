                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.course-videos.store") }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="course_material_id" id="course_material_id">
                                @foreach($course_materials as $id => $course_material)
                                    <option value="{{ $id }}" {{ old('course_material_id') == $id ? 'selected' : '' }}>{{ $course_material }}</option>
                                @endforeach
                            </select>
...
<script>
    ...
    url: '{{ route('frontend.course-videos.storeMedia') }}',
    maxFilesize: 200, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    headers: {
